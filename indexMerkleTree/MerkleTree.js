...
//initial node  bottom-up   
    ...
    //functions     
      ...
      calcLeafIndex(UidADDTid){   //calc leaflocation  
       ...
       putTransactionInTree(Uid,Tid,Treceipt) {//將交易放進樹當中
    ...
    // }
       ...
       getRootHash() { //拿到交易證據(代表所有交易紀錄256bits的唯一證據)
    ...
    // }
       ...
       getTransaction(Uid,Tid) {//拿到交易內容
         this.index = this.calcLeafIndex(Uid.concat(Tid));
        return this.nodes[this.index].getContent();
    }

       extractSlice(Uid,Tid){//拿到證據切片
         this.index = this.calcLeafIndex(Uid.concat(Tid));
...
class Node{
       ...
       this.rightChild = rightChild;
         ...
         if (leftChild === null || rightChild === null) { // leaf node
                ...
                this.isLeaf = true;
                this.contentDigest = keccak256("initial no data"); //initial hash value
